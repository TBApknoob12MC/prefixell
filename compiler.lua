...
function tdump(t) local r,s,i="",{{t,nil,0}},"  " while #s>0 do local o=s[#s] local c,k,d=o[1],o[2],o[3] local nk,v=next(c,k) if not k then r=r.."{\n" end if nk~=nil then o[2]=nk local l=i:rep(d+1) r=r..l..(type(nk)=="number" and "" or (type(nk)=="string" and nk:match("^[%a_][%w_]*$") and nk or "["..string.format("%q",nk).."]").." = ") if type(v)=="table" then table.insert(s,{v,nil,d+1}) else r=r..(type(v)=="string" and string.format("%q",v) or tostring(v))..",\n" end else r=r..i:rep(d).."}"..(#s>1 and ",\n" or "\n") table.remove(s) end end return r end
